...
# Strips emoji/pictographic characters from slide text. Slide text should
# be plain text only -- the one emoji per slide (hook or CTA) is handled
# separately via composited Twemoji images, since the handwriting font
# has no emoji glyphs and would otherwise show a broken "missing glyph"
# box if an emoji ended up inside the text itself (e.g. if the AI content
# generator ignores instructions and embeds one anyway).
_EMOJI_PATTERN = re.compile(
    ...
    "\U00002300-\U000023FF"  # misc technical (includes some clock/media symbols)
...
CANVAS_SIZE = (1080, 1920)          # TikTok's documented native photo resolution (9:16)
...
MAX_TEXT_WIDTH = 760                # horizontal margin so text never runs into the flowers
SAFE_ZONE_LEFT = 50                 # left margin used when centering text
SAFE_ZONE_RIGHT = 150               # extra right margin -- TikTok's like/comment/
                                     # profile icon column sits here, so text
                                     # centers within the space excluding it,
                                     # not the full canvas width
...
BRAND_GREEN = (58, 88, 56)  # sampled/adapted from the template's own vine green
BADGE_TEXT = "OT TIP"
BADGE_Y = 300  # sits in the clear band between the top flowers and text zone


def _draw_brand_badge(draw, render_size, scale):
    """
    Draws a small consistent "OT TIP" pill badge near the top of every
    slide, in the template's own green -- a fixed brand cue so the
    account is recognizable at a glance while scrolling, independent of
    whatever topic-specific text is on a given slide.
    """
    s = scale
    badge_font = ImageFont.truetype(FONT_PATH, int(44 * s))
    text_w = draw.textlength(BADGE_TEXT, font=badge_font)
    bbox = badge_font.getbbox(BADGE_TEXT)
    text_h = bbox[3] - bbox[1]

    pad_x, pad_y = 32 * s, 16 * s
    pill_w = text_w + pad_x * 2
    pill_h = text_h + pad_y * 2
    pill_x = SAFE_ZONE_LEFT * s
    pill_y = BADGE_Y * s
    ...
    draw.text(
        (pill_x + pad_x, pill_y + pad_y - bbox[1]),
        BADGE_TEXT,
        font=badge_font,
        fill=(255, 255, 255),
    )
...
def render_slide(text, output_path, emoji=None, show_badge=True):
    """
    ...
    `show_badge` draws the consistent "OT TIP" brand pill (on by default).
    """
    # Safety net: strip any stray emoji from the text itself, regardless
    # of what generated it. Only the explicit `emoji` argument gets
    # rendered (as a composited image) -- an emoji left inside `text`
    # would otherwise be drawn by the handwriting font, which has no
    # glyph for it and shows a broken box instead.
    ...
    if show_badge:
        _draw_brand_badge(draw, render_size, SUPERSAMPLE)
    ...
    for i, line in enumerate(lines):
        ...
        # Center within the safe zone (excludes TikTok's right-side icon
        # column) rather than the full canvas width.
        safe_left = SAFE_ZONE_LEFT * s
        safe_right = render_size[0] - SAFE_ZONE_RIGHT * s
        safe_width = safe_right - safe_left
        x = safe_left + (safe_width - total_w) / 2
    ...
    # Save as JPEG -- TikTok's photo-post endpoint does not support PNG
    # (per TikTok's own media requirements), only JPG/JPEG. Quality 95
    # keeps compression artifacts invisible at this resolution.
...
def render_slideshow(slide_texts, output_dir, hook_emoji=None, add_cta=True,
    ...
    for i, text in enumerate(all_slides, start=1):
        ...
        render_slide(text, path, emoji=emoji, show_badge=(i == 1))
        paths.append(path)
    return paths
