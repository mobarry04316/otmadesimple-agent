...
The FIRST slide (the hook) must make it obvious at a glance this is an OT \
post and state a clear payoff -- what the viewer will learn. Do one of:
  - Start with "OT students:" or "OT tip:"
  - Include the phrase "occupational therapy" or a specific OT term
    (fieldwork, ADLs, clinical reasoning, NBCOT, etc.)
Avoid generic self-help phrasing that could belong to any niche (e.g. \
"your goals are doing too much" -- too vague). Prefer something like \
"OT students: compensatory vs remediation, how do you actually pick?" \
over a generic version of the same idea.

IMPORTANT: the "slides" text strings must be PLAIN TEXT ONLY -- no emoji \
characters inside them, anywhere. The single hook emoji goes ONLY in the \
separate "hook_emoji" field; never embed an emoji directly in any slide's \
text, including the hook slide itself.
...
PERFORMANCE_SUMMARY_PATH = os.path.join(os.path.dirname(__file__), "performance_summary.json")
...
def generate_tip_set(client=None):
    ...
    itself, steered away from whatever's in the recent-history file, and
    informed by which past topics/hooks actually performed well (if
    performance_summary.json exists yet -- it won't for the first ~2
    weeks until enough posts have aged past TikTok's insights delay).
    """
    ...
    if performance and performance.get("top_performers"):
        user_content += "\n\nReal performance data from recent posts -- lean into the " \
                         "STYLE and ANGLE of top performers, avoid what underperformed " \
                         "(don't just repeat their exact topics, which are already in " \
                         "the avoid-list above):\n\nTop performing hooks:\n"
        for p in performance["top_performers"]:
            user_content += f"- \"{p['hook']}\" (topic: {p['topic']})\n"
        if performance.get("underperformers"):
            user_content += "\nUnderperforming hooks (avoid similar phrasing/angles):\n"
            for p in performance["underperformers"]:
                user_content += f"- \"{p['hook']}\" (topic: {p['topic']})\n"
